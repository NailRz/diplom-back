  ...
  async getErrorStats(userId: number): Promise<any> {
    ...
    results.forEach((result) => {
      ...
      result.mistakes.forEach((mistake, index) => {
        ...
        if (index < result.mistakes.length - 1 && result.mistakes[index + 1].wordIndex === mistake.wordIndex) {
          return;
        }
        ...
        for (let i = 0; i < currentWordErrors.length - 1; i++) {
          for (let j = i + 1; j < currentWordErrors.length; j++) {
            const combination = currentWordErrors.slice(i, j + 1)
              .map((error) => error.correctLetter)
              .join('');
            if (combination.length <= 3) {
              if (combinationErrors.has(combination)) {
                combinationErrors.set(combination, combinationErrors.get(combination) + 1);
              } else {
    ...
    const combinationErrorsObject = Object.fromEntries(combinationErrors.entries());
  
    return { letterErrors: letterErrorsArray, combinationErrors: combinationErrorsObject };
  }
